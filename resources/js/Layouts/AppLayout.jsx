...
const AppLayout = ({ children }) => {
    ...
    const isHome = url === '/subscriptions' || url.startsWith('/subscriptions?');
    const isAdd = url === '/add-subscription' || (url.includes('/subscriptions/') && url.endsWith('/edit'));
    ...
    return (
        <>
            ...
            <div className="pointer-events-none fixed inset-0 -z-10">
                <div className="absolute -top-24 -left-24 h-72 w-72 rounded-full bg-amber-200/60 blur-3xl dark:bg-amber-500/10"></div>
                <div className="absolute top-20 -right-16 h-80 w-80 rounded-full bg-sky-300/50 blur-3xl dark:bg-sky-500/10"></div>
                <div className="absolute bottom-10 left-1/3 h-64 w-64 rounded-full bg-rose-200/40 blur-3xl dark:bg-rose-500/10"></div>
            </div>
            ...
            <div className="fixed bottom-0 left-0 w-full z-50 pb-[var(--inset-bottom)]">
                <div className="max-w-5xl mx-auto px-5 pb-3">
                    <div className="app-card-strong flex items-center justify-between gap-2 px-3 py-2 bg-white/95 dark:bg-slate-900/95">
                        <Link
                            href="/subscriptions"
                            className={`flex flex-1 flex-col items-center justify-center gap-1 rounded-xl px-3 py-2 text-xs font-semibold transition-colors ${isHome ? 'bg-slate-100 text-slate-900 dark:bg-slate-800 dark:text-white' : 'text-slate-500 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white'}`}
                        >
                        ...
                        <Link
                            ...
                            className={`flex flex-1 flex-col items-center justify-center gap-1 rounded-xl px-3 py-2 text-xs font-semibold transition-colors ${isAdd ? 'bg-slate-100 text-slate-900 dark:bg-slate-800 dark:text-white' : 'text-slate-500 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white'}`}
                        >
                            <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                        ...
                        <Link
                            href="/settings"
                            className={`flex flex-1 flex-col items-center justify-center gap-1 rounded-xl px-3 py-2 text-xs font-semibold transition-colors ${isSettings ? 'bg-slate-100 text-slate-900 dark:bg-slate-800 dark:text-white' : 'text-slate-500 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white'}`}
                        >
                            <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" aria-hidden="true">
